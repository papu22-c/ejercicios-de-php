...
<body>
	<main>
		...
		<br>
		<form method='post' action='servicios/gestion.php'>
		  <div class="row mb-3">
		  ...
		  <br><br>
		  <span id="mensaje"><?=$mensaje ?? null ?></span>
		</form><br>
		...
		<table class="table table-striped">
			...
			<?php 
				foreach ($personas as $nifTabla=>$persona){
					...
					echo "<td class='nombre'>$persona[nombre]></td>";
					echo "<td class='direccion'>$persona[direccion]</td>";
					echo "<td><button type='button' class='btn btn-success' onclick='trasladarDatos(this)'>consulta</button></td>";       
					echo "</tr>";
				}
			?>
			
		    	<td >00000</td>
		      	<td><input id="nombreModi" type='text' value='<?=$nombreModi ?? null ?>' class='nombre'></td>
		      	<td><input id="direccionModi"type='text' value='000' class='direccion'></td>
			  	<input type='hidden' name='modificar'>
		      	<td>
		      	<form method='post' action='./servicios/gestion.php'>
		      		<input type='hidden' name='nifBaja' value='<?=$nif ?? null ?>'>
				</form>
				<button type="button" class="btn btn-warning" name='bajaPersona'>Baja</button>
				<button type="button" class="btn btn-primary" onclick='modificarPersonas(this)' name='modiPersona'>Modificar</button>
		...
		<form method='post' action='./servicios/gestion.php' id='formularioModi'>
